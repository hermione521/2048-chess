document.addEventListener("DOMContentLoaded", function () {
  	...
  	var host = 'ws://localhost:18080/';
  	//var host = 'ws://localhost:8080/game/sockets', sockjs, multiplexer;
 	...
 	window.requestAnimationFrame(function () {
	 	var startNewGame = function () {
	 		...
	 		$('#game-start-btn').on('click', function () {
				...
				userNumInterval = setInterval(function () {
					$.get('http://localhost:8080/game/players', function (data) {
		...
		var startGame = function () {
			...
			io.onmessage = function(event) {
			    ...
			    //}
			    for (var i = window._io.listeners.length - 1; i >= 0; i--) {
			    	window._io.listeners[i](msg);
			    }
				    ...
				    var countdown = setInterval(function() {
						...
						$('#player-msg').removeClass('text-center');	
				   		...
				   		if (times === -1) {
				   			...
				   			var localManager = new GameManager({
					   				size: window._gameBoard.size,
					   				startTiles: window._gameBoard.startTiles,
					   				player: window._gameBoard.player,
					   				otherPlayer: opposingPlayer,},
				   				HTMLActuator,
