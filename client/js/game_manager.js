function GameManager(options, Actuator, Keyboard, Online, socket) {
  ...
  this.onlineManager = new Online();

  this.keyboardManager.on("move", function (direction) {
    socket.send(JSON.stringify({player: options.player, move: direction}));
  });

  this.onlineManager.on("move", this.move.bind(this));
  this.onlineManager.on("restart", this.restart.bind(this));
...
GameManager.prototype.restart = function () {
  ...
  this.actuator.actuate(this.grid, {myScore: 0, opScore:0, over: false, won: false});
...
GameManager.prototype.actuate = function (cb) {
  var self = this;
...
GameManager.prototype.move = function (msg) {
  if (typeof(msg.tile) != 'undefined'){
    newTile = new Tile(msg.tile.cell, msg.tile.value);
    this.grid.insertTile(newTile);
    return;
  }
  var direction = msg.move;
  var moveGamer = msg.player;
    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && next.value === tile.value && !next.mergedFrom) {
          ...
          // Update the score
          if (moveGamer === self.options.player){
            self.myScore += merged.value;
            window._io.player[moveGamer] = self.myScore;
          }
          else{
            self.opScore += merged.value;
            window._io.player[moveGamer] = self.opScore;
          }
  ...
  self.actuate(function () {
    console.log("actuated.");
    if (moved) {
      if (moveGamer === self.options.player) {
        self.addRandomTile(function () {
          self.actuate(function (){
            console.log('player ' + moveGamer + ' add random tile!!!');
          });
        });
      }
    }
